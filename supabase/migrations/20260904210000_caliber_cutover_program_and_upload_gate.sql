...
--    status + age + attempts, so relaxing the rule unconditionally would have
--    swept ~3,439 already-skipped historical rows per week into Google in one
--    batch and spiked Smart Bidding. Rows created before the cutover timestamp
--    keep the old allow-list exactly; only new rows use the deny-list.
...
-- The moment the deny-list takes effect. Kept as a function, not a literal
-- inlined into the view, so the date can be moved with a one-line CREATE OR
-- REPLACE instead of a view rebuild — useful if the Energy cutover slips.
create or replace function public.offline_cv_denylist_cutover()
returns timestamptz
language sql
immutable
as $$ select timestamptz '2026-09-05 00:00:00+00' $$;
...
comment on function public.offline_cv_denylist_cutover() is
  ...
  'events keep the historical allow-list. Move it to re-date the cutover.';
